...
def genera_html(risultati, stats, cap_labs, cap_vals, sp500_vals, drawdown_vals, max_dd,
    ...
    html = (
        ...
        '  <div class="h-actions">\n'
        '    <div class="h-btns">\n'
        '      <button class="act-btn tg-btn" onclick="sendTelegram()">&#x1F4F1; Invia Telegram</button>\n'
        '      <button class="act-btn upd-btn" onclick="aggiornaBacktest()">&#x1F504; Aggiorna dati</button>\n'
        '      <button class="act-btn" style="background:#6e40c9;color:#fff" onclick="showTokenModal()">&#x2699;&#xFE0F; Token</button>\n'
        '    </div>\n'
        '    <div class="act-status" id="act-status"></div>\n'
        '  </div>\n'
        '</header>\n'
        '\n'
        ...
        '\n'
        'const GH_REPO = "tobiatidesca-art/etf-momentum";\n'
        'const GH_WORKFLOW = "aggiornamento.yml";\n'
        'function getToken() { return localStorage.getItem("gh_pat") || ""; }\n'
        'function showTokenModal() {\n'
        '  const t = prompt("GitHub Personal Access Token (scope: workflow):", getToken());\n'
        '  if (t !== null) { localStorage.setItem("gh_pat", t.trim());\n'
        '    document.getElementById("act-status").textContent = t.trim() ? "Token salvato." : "Token rimosso."; }\n'
        '}\n'
        'async function triggerWorkflow(inviaTelegram) {\n'
        ...
        '    body:JSON.stringify({ref:"main",inputs:{invia_telegram:String(inviaTelegram)}})\n'
        '  });\n'
        '  return r.status === 204;\n'
        '}\n'
        ...
        '  const ok = await triggerWorkflow(true).catch(()=>false);\n'
        '  if (ok) { btn.textContent="Avviato!"; st.textContent="Workflow avviato \u2014 Telegram in arrivo in ~2 min."; }\n'
        '  else    { btn.textContent="Errore";   st.textContent="Controlla il Token GitHub (ingranaggio)."; }\n'
        '  setTimeout(() => { btn.innerHTML = "&#x1F4F1; Invia Telegram"; btn.disabled = false; }, 5000);\n'
        '}\n'
        'async function aggiornaBacktest() {\n'
        '  const btn = document.querySelector(".upd-btn");\n'
        '  const st  = document.getElementById("act-status");\n'
        '  btn.disabled = true; btn.textContent = "Avvio...";\n'
        '  st.textContent = "Avvio workflow GitHub Actions...";\n'
        '  const ok = await triggerWorkflow(false).catch(()=>false);\n'
        '  if (ok) { btn.textContent="Avviato!"; st.textContent="Aggiornamento avviato \u2014 la pagina si aggiorner\u00e0 in ~3 min."; }\n'
        '  else    { btn.textContent="Errore";   st.textContent="Controlla il Token GitHub (ingranaggio)."; }\n'
        '  setTimeout(() => { btn.innerHTML = "&#x1F504; Aggiorna dati"; btn.disabled = false; }, 5000);\n'
        '}\n'
        '</script>\n'
        '</body>\n'
        '</html>\n'
