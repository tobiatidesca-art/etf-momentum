...
def genera_html(risultati, stats, cap_labs, cap_vals, sp500_vals, drawdown_vals, max_dd,
    ...
    html = (
        ...
        '  if (ok) { btn.textContent="Avviato!"; st.textContent="Telegram in arrivo in ~2 min."; }\n'
        '  else    { btn.textContent="Errore";   st.textContent="Errore avvio workflow."; }\n'
        ...
        '  const st  = document.getElementById("act-status");\n'
        '  btn.disabled = true; btn.textContent = "Avvio...";\n'
        '  st.textContent = "Avvio aggiornamento...";\n'
        '  const ok = await triggerWorkflow(false).catch(()=>false);\n'
        '  if (ok) { btn.textContent="Avviato!"; st.textContent="Aggiornamento in corso \u2014 pagina aggiornata in ~3 min."; }\n'
        '  else    { btn.textContent="Errore";   st.textContent="Errore avvio workflow."; }\n'
